...
public class LoginController {
	...
	@RequestMapping("/logins/adminlogin")
    public String getToAdminloginPage() {
		
		
        return "logins/adminlogin";
	 ...
	 * 管理员登录控制器
	 */
	@RequestMapping("/logins/login/Admin")
	   public String adminlogin(HttpServletRequest request,HttpServletResponse response,Administrator entity,HttpSession httpSession )
		{      
		...
		if((2<username.length()&username.length()<13)&&(2<password.length()&password.length()<13)){   
			if(adminDao.adminLogin(request.getParameter("usernamee"))!=null){
				entity = adminDao.adminLogin(request.getParameter("usernamee"));
				if(entity.getPassword().compareTo(request.getParameter("passwd"))==0){
					System.out.println("登录成功");
					  return "redirect:/users/Name="+username; 
		                ...
		                return "redirect:/logins/adminlogin";
					}
			}
	   ...
	   public String loginthree(HttpServletRequest request,HttpServletResponse response,User entity,HttpSession httpSession )
		...
		if((2<username.length()&username.length()<13)&&(2<password.length()&password.length()<13)){   
			if(ecuserDaoThree.findByUsernameFromaccountone(request.getParameter("usernamee"))!=null){
				...
				if(entity.getPassword().compareTo(request.getParameter("passwd"))==0){
					System.out.println("登录成功");
					  return "redirect:/known/Name="+username; 
